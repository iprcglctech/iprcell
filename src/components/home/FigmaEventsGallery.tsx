...
export default function FigmaEventsGallery() {
  ...
  return (
    <section className="min-h-[calc(100vh-72px)] min-h-[calc(100svh-72px)] flex flex-col justify-center py-6 sm:py-8 lg:py-10 bg-navy-900 text-white border-b border-navy-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-4 sm:space-y-6 w-full my-auto">
        {/* Section Header with ScrollReveal */}
        <ScrollReveal delay={0.1} className="flex flex-col md:flex-row md:items-end justify-between gap-3">
          <div className="space-y-1 sm:space-y-1.5">
            <div className="text-xs uppercase font-bold tracking-[0.2em] text-electric-light font-sans">
              EVENTS &amp; FLAGSHIPS
            </div>
            <h2 className="text-xl sm:text-2xl lg:text-3xl font-serif font-normal text-white tracking-tight">
          ...
          <div>
            <Link
              href="/events"
              className="inline-flex items-center space-x-2 text-xs uppercase font-bold tracking-widest text-slate-200 hover:text-electric-light transition-colors group border border-white/20 hover:border-electric px-3.5 py-2 rounded"
            >
        ...
        <ScrollReveal delay={0.2} yOffset={20} className="pt-1 flex justify-center items-center">
          <DepthCarousel
            items={eventsGallery}
            intervalMs={2500}
            autoPlay={true}
